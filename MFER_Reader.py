# All rights reserved by CardioIntelligence Inc.
...
class MFER_12Leads:
    def __init__(self, mferfile, codes_path=None, show_attrib=True):
        """
        MFERファイルを読み込むクラス
        ...
        codes_path : str, optional
            codes.xlsxファイルのパス。指定がない場合は同じディレクトリから探す
        show_attrib : bool, default True
            属性情報を表示するかどうか
        """
        # codes.xlsxのパスを設定
        if codes_path is None:
            codes_path = os.path.join(os.path.dirname(__file__), "codes.xlsx")
        
        if not os.path.exists(codes_path):
            raise FileNotFoundError(f"コード定義ファイルが見つかりません: {codes_path}")
            
        if not os.path.exists(mferfile):
            raise FileNotFoundError(f"MFERファイルが見つかりません: {mferfile}")
        ...
        for wave_idx in range(2):
            ...
            for chn in range(8):  # channel_num = 8
                start_idx = block_length * 2 * chn
                end_idx = block_length * 2 * (chn + 1)
                wave_data = wave_data_lists[wave_idx][0][start_idx:end_idx]
                
                decoded_data = decode_data(wave_data, 2) * 0.00125  # sampling_sen
                records.append({
                    lead_lists[wave_idx][chn]: decoded_data
                })
                
                del wave_data
                gc.collect()

            dfwave = pl.DataFrame(records[0])
            for record in records[1:]:
                dfwave = dfwave.with_columns(pl.Series(name=list(record.keys())[0], values=list(record.values())[0]))
            
            dfwave = dfwave.with_columns([
                (pl.col("MWF_ECG_LEAD_I") - pl.col("MWF_ECG_LEAD_II")).alias("MWF_ECG_LEAD_III"),
                (-(pl.col("MWF_ECG_LEAD_I") + pl.col("MWF_ECG_LEAD_II"))/2).alias("MWF_ECG_LEAD_aVR"),
                (pl.col("MWF_ECG_LEAD_I") - pl.col("MWF_ECG_LEAD_II")/2).alias("MWF_ECG_LEAD_aVL"),
                (pl.col("MWF_ECG_LEAD_II") - pl.col("MWF_ECG_LEAD_I")/2).alias("MWF_ECG_LEAD_aVF"),
                pl.Series(name="offset_msec", values=np.arange(len(dfwave)) * 2)  # sampling_ivl = 2
            ])

            waveforms.append(dfwave.to_pandas())
        ...
        try:
            ...
            raise RuntimeError(f"コード定義ファイルの読み込みに失敗しました: {str(e)}")
